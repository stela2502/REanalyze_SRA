...
ok ( -f "$plugin_path/data/output/PRJEB7858_SUMMARY.xls", "the input summary file exists" );

die "Test of script $exec not possible - please run $plugin_path/stefans_libs_XML_parser.t first\n" unless ( -f "$plugin_path/data/output/PRJEB7858_SUMMARY.xls");

my $cmd =
    "perl -I $plugin_path/../lib  $exec "
  . " -bam_files ". join(" ", @files)
...
open ( LOG , "<$plugin_path/data/output/Script/PRJEB7858.R.log" ) or die "I could not open the log file\n$!\n";
...
my $data_table = data_table->new( {'filename' => "$plugin_path/data/output/Script/Samples.xls"} );

($internal_vals->{'filename col'}) = $data_table->Header_Position('filename');

ok ( defined $internal_vals->{'file -> table column:'}, "log value 1 OK ($internal_vals->{'file -> table column:'})");

for ( my $i = 0; $i < $data_table->Rows(); $i ++ ) {
	@tmp = @{@{$data_table->{'data'}}[$i]};
	ok ( $tmp[$internal_vals->{'filename col'}] =~ m/$tmp[$internal_vals->{'file -> table column:'}]/, "file $tmp[$internal_vals->{'filename col'}] matches corresponding accession number $tmp[$internal_vals->{'file -> table column:'}] in Samples.xls file" );
}
